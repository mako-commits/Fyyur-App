...
def index():
  ...
  new_artists = Artist.query.order_by(desc(Artist.created_at)).limit(10).all()
...
def show_venue(venue_id):
  ...
  past_shows_query = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time<present_time).all()
  past_shows=[]
  ...
  upcoming_shows_query = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time>present_time).all()
  upcoming_shows = []
...
def show_artist(artist_id):
  ...
  past_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time<present_time).all()
  past_shows = []
  ...
  upcoming_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time>present_time).all()
  upcoming_shows = []
